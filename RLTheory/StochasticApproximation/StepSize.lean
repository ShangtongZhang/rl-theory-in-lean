...
lemma anchors_of_inv_poly {ν : ℝ} (hν : ν ∈ Set.Ioo (2 / 3) 1) :
  ...
  have : 0 < (1 - ν)⁻¹ := by apply inv_pos_of_pos; linarith
  ...
  have hzν : (1 - z) * -((1 - ν)⁻¹ * ν) ≤ -(z * 2) := by
    ...
    linarith
    apply mul_right_cancel₀ (b := 1 - ν)
    linarith
    rw [add_mul, mul_assoc, inv_mul_cancel₀, mul_comm (b := 2 - ν),
  ...
  have : 0 < (-ν + 1) / (-z + 1) := by
    ...
    linarith
    linarith
  have hnrpow : ∀ n, 1 ≤ n → 0 < ((n : ℝ) + 1) ^ (1 - z) - 1 := by
    intro n hn
    simp
    apply one_lt_rpow
    simp
  ...
  have : 0 < η := by
    ...
    have := (rpow_lt_rpow_left_iff (x := 2) (by simp)).mpr this
    simp at this
    exact this
  have hη : ∀ n, 1 ≤ n → ((n : ℝ) + 1) ^ (1 - z) - 1 ≥ η * (n + 1) ^ (1 - z)
    := by
    intro n hn
    simp
    ...
    simp [←one_sub_mul, η]
    nth_rw 1 [←rpow_zero 2]
    have : 0 = z - 1 + (1 - z) := by ring
    rw [this, rpow_add]
    apply mul_le_mul_of_nonneg_left
    have : 2 ≤ n + 1 := by linarith
    grw [this]
    linarith
    positivity
    simp
  set T := fun n : ℕ => inv_poly z 1 n
  set anc : Anchors (α := α) := by
    constructor
    case T => exact T
    case hα =>
      apply robbinsMonro_of_inv_poly
      constructor; linarith; linarith
      simp
    case hα_mono =>
      intro x y hxy
      simp [α, inv_poly]
      apply Real.rpow_neg_le_rpow_neg_of_le
      linarith
      linarith
      linarith
      simp [hxy]
    case hT =>
      apply robbinsMonro_of_inv_poly
      constructor; exact hz.1; linarith
      rfl
  have : ∃ C, 0 ≤ C ∧ ∀ (n : ℕ), α (anc.t n) ≤ C * anc.T n ^ 2 := by
    set C₁ :=
      ((-ν + 1) / (-z + 1)) ^ (-((-ν + 1)⁻¹ * ν)) * η ^ (-((-ν + 1)⁻¹ * ν))
    refine ⟨?C, ?hCnonneg, ?hC⟩
    case C => exact max C₁ (α 0 / T 0 ^ 2)
    case hCnonneg =>
      apply LE.le.trans ?_ (by apply le_max_right)
      simp [α, inv_poly, T]
      positivity
    case hC =>
      intro n
      ...
      have hf : AntitoneOn f (Set.Icc 0 (0 + ↑(anc.t n))) := by
        intro x hx y hy hxy
        simp [f, inv_poly]
        ring_nf
        apply Real.rpow_neg_le_rpow_neg_of_le
      ...
      have hf : AntitoneOn f (Set.Icc 0 (0 + ↑n)) := by
        intro x hx y hy hxy
        simp [f]
        apply Real.rpow_neg_le_rpow_neg_of_le
        have := hz.1
        simp [zmin] at this
        linarith
        simp at hx; simp; linarith
        simp at hy; simp; linarith
        linarith
      ...
      simp [this] at hineq
      have hineq := Real.div_mul_le_of_div_le_div (by linarith) hineq
      have hineq := Real.le_of_le_sub_nonneg (by linarith) hineq
      have hineq := Real.rpow_le_of_le_rpow
        (by linarith) ?_ (by linarith) hineq
      have : (anc.t n : ℝ) - 1 + 2 ≤ (anc.t n : ℝ) + 2 := by linarith
      simp only [inv_poly, anc, T] at this
      have hineq := (hineq.trans_eq (by simp)).trans this
      have hineq := Real.rpow_neg_le_rpow_neg_of_le (r := ν)
      ...
      have := Real.rpow_neg_le_rpow_neg_of_le
        (r := ((1 - ν)⁻¹ * ν)) ?_ ?_ ?_ this
      grw [this]
      rw [mul_rpow]
      apply mul_le_mul_of_nonneg_left
      ...
      linarith
      simp_rw [neg_add_eq_sub]
      apply mul_nonneg; apply div_nonneg; linarith; linarith; linarith
      apply rpow_pos_of_pos
      simp_rw [neg_add_eq_sub]
      apply mul_pos; apply div_pos; linarith; linarith; linarith
      linarith
      simp_rw [neg_add_eq_sub]
      apply mul_nonneg; apply div_nonneg; linarith; linarith; linarith
